...
class MetropolisHastingsIterator:
    ...
    def __init__(self, initialState):
        ...
        :param initialState: the initial state of the Markov process
...
def MetropolisHastings(
    ...
    _ = Parallel(n_jobs=number_of_parallel_jobs, prefer='threads')(
        delayed(generate_draws)(i, warmup) for i in iterators
    )
    ...
    for trials in range(maxNumberOfIterations):
        ...
        R_hat, neff, phi_bar = zip(
            *[AnalyzeDraws(draw_per_indicator) for draw_per_indicator
              in draws_analyse]
        )
        ...
        if np.all(R_hat <= R_hat_threshold) and np.all(neff >= target_neff):
