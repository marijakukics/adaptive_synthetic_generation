...
def stats_assessment(original_data, synthetic_data, aggregation_level=1,
                     ignore_cols=[], continuous_columns=[], sep='::',
                     stats_name=None):
    """
    Compute all the stats on the different combinations based on the
    aggregation_level.
    ...
    for c in combs:
        ...
        real_and_sampled = pd.merge(orig, synth, suffixes=['_orig', '_synth'],
                                    on=agg_vars, how='outer', indicator=True)
        real_and_sampled = real_and_sampled[['count_orig',
                                             'count_synth']].fillna(0)

        sts = compute_stats(real_and_sampled['count_orig'],
                            real_and_sampled['count_synth'],
...
def get_combinations(columns, aggregation_level, sep='::'):
    ...
    combinations_list: list[str]
        The list of combinations of columns. Each element corresponds to one
        combinations.
    """

    if aggregation_level not in [1, 2, 3]:
        raise ValueError("The variable 'aggregation_level' can only take the \
                          values 1, 2, or 3.")
...
def compute_stats(freq_list_orig, freq_list_synth, stats_name=None):
    """
    Compute different statistics (MAE, RMSE, SMRSE, R^2, and Pearson's
    correlation) on two frequency lists.
    ...
    freq_list_orig, freq_list_synth = np.array(freq_list_orig), np.array(
        freq_list_synth)
    if stats_name is None:
        stats_name = ['mae', 'rmse', 'r2', 'srmse', 'corr']
    stat = {}
    if 'mae' in stats_name:
        stat['mae'] = np.absolute(freq_list_orig - freq_list_synth).mean()
    if 'rmse' in stats_name:
        stat['rmse'] = np.linalg.norm(freq_list_orig-freq_list_synth)/np.sqrt(
            len(freq_list_orig))
    if 'srmse' in stats_name:
        freq_list_orig_avg = freq_list_orig.mean()
        rmse = np.linalg.norm(freq_list_orig - freq_list_synth) / np.sqrt(
            len(freq_list_orig))
        stat['srmse'] = rmse / freq_list_orig_avg
    if 'r2' in stats_name:
        u = np.sum((freq_list_synth - freq_list_orig) ** 2)
        v = np.sum((freq_list_orig - freq_list_orig_avg) ** 2)
        stat['r2'] = 1 - u / v
    if 'corr' in stats_name:
